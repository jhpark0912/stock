...
> - **유지보수 전략**: `.claude/plans/MAINTENANCE_STRATEGY.md`
...
### 2차 (P3 2차 대상에서 추출, 2026-02-23 추가)

| 훅 | 역할 | 추출 원본 |
|----|------|----------|
| `useSectorDetail(symbol)` | 섹터 종목 데이터 조회 + 정렬 상태 | `SectorDetail.tsx` |
| `useAIAnalysis(ticker)` | AI 분석 호출 + 스트리밍 상태 관리 | `AIAnalysisTab.tsx` |
| `useSectorHeatmap()` | 히트맵 데이터 가공 + 필터링 (hook 밀도 10회, 우선 추출 권장) | `SectorHeatmap.tsx` |
