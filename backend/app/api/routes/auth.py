...
def register(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    if user_repo.exists(user_data.username):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="이미 사용 중인 사용자명입니다")

    # 비밀번호 해싱
    password_hash = AuthService.hash_password(user_data.password)

    # 사용자 생성 (is_approved=False)
    user_repo.create(username=user_data.username, password_hash=password_hash, role="user", is_approved=False)
...
def login(login_data: UserLogin, db: Session = Depends(get_db)):
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="사용자명 또는 비밀번호가 잘못되었습니다")
    ...
    if not user.is_approved:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="승인 대기 중입니다. 관리자 승인 후 로그인이 가능합니다."
        )
...
def update_gemini_key(
    key_data: GeminiKeyUpdate, current_user: UserDB = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Gemini API 키 설정"""
    repo = UserRepository(db)
    repo.update_gemini_key(current_user.id, key_data.api_key)
    return GeminiKeyStatus(has_key=True, key_preview=f"{key_data.api_key[:4]}...{key_data.api_key[-4:]}")


@router.delete("/gemini-key")
def delete_gemini_key(current_user: UserDB = Depends(get_current_user), db: Session = Depends(get_db)):
    """Gemini API 키 삭제"""
    repo = UserRepository(db)
...
def get_gemini_key_status(current_user: UserDB = Depends(get_current_user), db: Session = Depends(get_db)):
    """Gemini API 키 상태 조회"""
    repo = UserRepository(db)
...
def update_kis_credentials(
    credentials: KISCredentialsUpdate, current_user: UserDB = Depends(get_current_user), db: Session = Depends(get_db)
):
    """한국투자증권 API 인증정보 설정"""
    repo = UserRepository(db)
    repo.update_kis_credentials(current_user.id, credentials.app_key, credentials.app_secret)
...
def delete_kis_credentials(current_user: UserDB = Depends(get_current_user), db: Session = Depends(get_db)):
    """한국투자증권 API 인증정보 삭제"""
    repo = UserRepository(db)
...
def get_kis_credentials_status(current_user: UserDB = Depends(get_current_user), db: Session = Depends(get_db)):
