...
from app.models.economic import CommoditiesData, EconomicIndicator, HistoryPoint, RatesData
...
def get_commodities_data(include_history: bool = False) -> CommoditiesData:
    ...
    return CommoditiesData(
        wti_oil=get_yahoo_indicator("CL=F", include_history), gold=get_yahoo_indicator("GC=F", include_history)
    )
...
def get_all_yahoo_indicators(include_history: bool = False) -> Dict[str, Any]:
    ...
    """
    return {"rates": get_rates_data(include_history), "commodities": get_commodities_data(include_history)}
...
def get_all_yahoo_indicators_parallel(include_history: bool = False) -> Dict[str, Any]:
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        future_to_symbol = {executor.submit(get_yahoo_indicator, sym, include_history): sym for sym in symbols}
    ...
    return {
        "rates": RatesData(treasury_10y=results.get("^TNX"), treasury_3m=results.get("^IRX"), vix=results.get("^VIX")),
        "commodities": CommoditiesData(wti_oil=results.get("CL=F"), gold=results.get("GC=F")),
    }
