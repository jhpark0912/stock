...
async def get_etf_holdings(etf_code: str, app_key: str, app_secret: str) -> Optional[List[KISHolding]]:
    """
    ETF 구성종목시세 조회
    ...
    try:
        ...
        for item in output2:
            try:
                ...
                holdings.append(KISHolding(symbol=symbol, name=name, price=price, change_1d=change_1d, weight=weight))
...
async def get_market_cap_ranking(
    ...
    try:
        ...
        # 시장 코드 매핑
        market_code = {"KOSPI": "0001", "KOSDAQ": "1001"}.get(market, "0001")
...
async def test_credentials(app_key: str, app_secret: str) -> Tuple[bool, Optional[str]]:
    ...
    """
    try:
        _get_access_token(app_key, app_secret)
        return (True, None)
