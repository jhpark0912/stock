...
async def get_fluctuation_ranking(market: str = "ALL", limit: int = 5) -> Tuple[List[PykrxStock], List[PykrxStock]]:
    """
...
async def get_market_cap_ranking(market: str = "KOSPI", limit: int = 5) -> List[PykrxStock]:
    """
    시가총액 순위 조회 (Top N)
...
async def get_stock_info(tickers: List[str], date: Optional[str] = None) -> Dict[str, PykrxStock]:
    """
    개별 종목 시세 조회
...
async def get_etf_portfolio(etf_ticker: str, limit: int = 10) -> List[Dict[str, Any]]:
    """
    ETF 구성종목 조회 (추정)
