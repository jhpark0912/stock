...
def judge_season_indpro(
    indpro_yoy: float, indpro_trend: str, cpi: float, vix: float, yield_spread: Optional[float] = None
) -> tuple[str, float]:
    """
    시장 사이클 판단 (INDPRO 기반)
...
def calculate_confidence(season: str, score: float, indpro_trend: str, cpi: float, vix: float) -> int:
    """
    신뢰도 계산 (0-100)
...
def generate_reasoning(season: str, indpro_yoy: float, indpro_trend: str, cpi: float, vix: float) -> str:
    """
    시장 사이클 판단 근거 생성
    ...
    reasoning = (
        f"{indpro_desc}, {cpi_desc}(CPI {cpi:.1f}%), {vix_desc}(VIX {vix:.1f})로 {season_names[season]}로 판단됩니다."
    )
...
def analyze_market_cycle(
    ...
    try:
        ...
        confidence = calculate_confidence(
            season=season, score=score, indpro_trend=indpro_trend, cpi=cpi_current, vix=vix_current
        )
        ...
        reasoning = generate_reasoning(
            season=season, indpro_yoy=indpro_yoy, indpro_trend=indpro_trend, cpi=cpi_current, vix=vix_current
        )
        ...
        cycle_data = MarketCycleData(
            ...
            reasoning=reasoning,
            indpro=MarketCycleIndicator(value=indpro_yoy, trend=indpro_trend, label="INDPRO (YoY)"),
            cpi=MarketCycleIndicator(
                value=cpi_current, trend=vix_trend_value, label="CPI (YoY)", mom_change=cpi_mom_str
            ),
            vix=MarketCycleIndicator(value=vix_current, trend=vix_trend, label="VIX"),
            yield_spread=yield_spread,
        )
...
def get_sample_market_cycle() -> MarketCycleData:
    ...
    return MarketCycleData(
        ...
        indpro=MarketCycleIndicator(value=2.5, trend="상승 추세", label="INDPRO (YoY)"),
        cpi=MarketCycleIndicator(value=3.1, trend="상승 추세", label="CPI (YoY)", mom_change="+0.2"),
        vix=MarketCycleIndicator(value=18.5, trend="낮은 변동성", label="VIX"),
        yield_spread=45.0,
    )
...
def generate_ai_comment(cycle_data: MarketCycleData, api_key: str) -> Dict[str, str]:
    """
