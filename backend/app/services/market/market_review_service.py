...
async def get_kr_top_movers(
    kis_app_key: Optional[str] = None, kis_app_secret: Optional[str] = None
    ...
    if kis_app_key and kis_app_secret:
        try:
            ...
            kis_gainers, kis_losers = await kis_get_fluctuation(
                app_key=kis_app_key, app_secret=kis_app_secret, market="ALL", limit=5
            )
    ...
    # 2. pykrx fallback
    try:
        from app.services.korea_data.pykrx_service import get_fluctuation_ranking as pykrx_get_fluctuation
        from app.services.korea_data.pykrx_service import is_pykrx_available
...
async def get_kr_major_stocks(
    ...
    if kis_app_key and kis_app_secret:
        try:
            ...
            if kospi_result or kosdaq_result:
                ...
                logger.debug(
                    f"[MarketReview] KIS API 시총 조회 성공: KOSPI {len(kospi_stocks)}개, KOSDAQ {len(kosdaq_stocks)}개"
                )
    ...
    # 2. pykrx fallback
    try:
        from app.services.korea_data.pykrx_service import get_market_cap_ranking as pykrx_get_market_cap
        from app.services.korea_data.pykrx_service import is_pykrx_available
...
async def get_market_review(
    country: str, kis_app_key: Optional[str] = None, kis_app_secret: Optional[str] = None
) -> MarketReviewData:
    """
...
async def generate_market_review_ai(
    ...
    try:
        ...
        gainers_text = "\n".join(
            [f"- {g.name}: {'+' if g.change_percent > 0 else ''}{g.change_percent:.2f}%" for g in data.top_gainers[:3]]
        )

        losers_text = "\n".join([f"- {l.name}: {l.change_percent:.2f}%" for l in data.top_losers[:3]])
