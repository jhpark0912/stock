...
def _set_cache(key: str, data: Any, ttl: int) -> None:
    """캐시 저장"""
    _korea_sector_cache[key] = {"data": data, "expires": datetime.now() + timedelta(seconds=ttl)}
...
KOREA_SECTOR_ETFS = {
    ...
    "091170.KS": {"name": "은행", "name_en": "Banks", "description": "KB금융, 신한지주, 하나금융"},
    "266420.KS": {"name": "헬스케어", "name_en": "Healthcare", "description": "삼성바이오, 셀트리온, 유한양행"},
    "117460.KS": {
        "name": "에너지화학",
        "name_en": "Energy & Chemicals",
        "description": "LG화학, SK이노베이션, 롯데케미칼",
    },
    "266370.KS": {"name": "IT", "name_en": "Information Technology", "description": "네이버, 카카오, 삼성SDI"},
    "091180.KS": {"name": "자동차", "name_en": "Automobiles", "description": "현대차, 기아, 현대모비스"},
    "117700.KS": {"name": "건설", "name_en": "Construction", "description": "삼성물산, 현대건설, DL이앤씨"},
    "140710.KS": {"name": "운송", "name_en": "Transportation", "description": "HMM, 대한항공, 한진칼"},
    "102970.KS": {"name": "증권", "name_en": "Securities", "description": "미래에셋증권, 한국금융지주, 삼성증권"},
    "266390.KS": {
        "name": "경기소비재",
        ...
        for symbol, meta in KOREA_SECTOR_ETFS.items():
            try:
                ...
                if symbol in history.index.get_level_values(0):
                    ...
                    if len(closes) >= 2:
                        # 1일 변화율 (어제 대비)
                        change_1d = _calculate_change_percent(closes.iloc[-1], closes.iloc[-2])

                        # 1주 변화율 (5거래일 전 대비)
                        if len(closes) >= 6:
                            change_1w = _calculate_change_percent(closes.iloc[-1], closes.iloc[-6])
                        else:
                            change_1w = change_1d

                        # 1개월 변화율 (첫 데이터 대비)
                        change_1m = _calculate_change_percent(closes.iloc[-1], closes.iloc[0])
                    else:
...
async def get_korea_sector_holdings(
    symbol: str, kis_credentials: Optional[tuple[str, str]] = None
    ...
    try:
        from app.services.korea_data.pykrx_service import get_data_date, get_etf_portfolio, is_pykrx_available
