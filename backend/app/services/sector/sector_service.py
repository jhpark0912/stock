...
def _set_cache(key: str, data: Any, ttl: int):
    """캐시에 데이터 저장"""
    _sector_cache[key] = {"data": data, "expires": time.time() + ttl}
...
def _get_holdings_from_db(symbol: str) -> Optional[List[str]]:
    ...
    db = SessionLocal()
    try:
        cache = db.query(SectorHoldingsCacheDB).filter(SectorHoldingsCacheDB.symbol == symbol).first()
...
def _save_holdings_to_db(symbol: str, top_holdings: List[str]) -> bool:
    ...
    try:
        ...
        if cache:
            ...
            # 신규 생성
            cache = SectorHoldingsCacheDB(symbol=symbol, top_holdings=holdings_json)
            db.add(cache)
...
SECTOR_ETFS = {
    ...
    "XLF": {"name": "금융", "name_en": "Financials", "description": "은행, 보험, 자산운용", "top_stock": "JPM"},
    "XLV": {"name": "헬스케어", "name_en": "Healthcare", "description": "제약, 바이오, 의료기기", "top_stock": "UNH"},
    "XLE": {"name": "에너지", "name_en": "Energy", "description": "석유, 가스, 에너지장비", "top_stock": "XOM"},
    "XLI": {"name": "산업재", "name_en": "Industrials", "description": "항공, 건설, 기계", "top_stock": "GE"},
    "XLB": {"name": "소재", "name_en": "Materials", "description": "화학, 금속, 건축자재", "top_stock": "LIN"},
    "XLY": {
        "name": "경기소비재",
    ...
    "XLRE": {"name": "부동산", "name_en": "Real Estate", "description": "REITs, 부동산개발", "top_stock": "AMT"},
    "XLU": {"name": "유틸리티", "name_en": "Utilities", "description": "전력, 가스, 수도", "top_stock": "NEE"},
    "XLC": {
        "name": "커뮤니케이션",
